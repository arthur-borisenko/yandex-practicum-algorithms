...
class TestCase(unittest.TestCase):
    ...
    def _dfs(cls, node):
        if node.right is not None and node.left is not None:
            return cls._dfs(node.left) + [node.value] + cls._dfs(
                node.right)
    ...
    def test_add(self):
        ...
        self.assertEqual(self._dfs(test_tree.root),
                         sorted(self._dfs(test_tree.root)))
    ...
    def test_add_illegal_value(self):
        ...
        try:
            ...
            self.assertFalse(True,
                             "error is not raised on already added node")
        except ValueError:
            self.assertTrue(True)
    @classmethod
    def _check_height_single(cls, node):
        return node.height == max(node.left.height if node.left is not None else 0, node.right.height if node.right is not None else 0)+1
    @classmethod
    def _check_height(cls, node):
        if node.right is not None and node.left is not None:
            return cls._check_height(node.right) and cls._check_height(node.left) and cls._check_height_single(node)
        elif node.left is not None:
