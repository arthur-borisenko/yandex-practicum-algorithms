...
def preprocess_text(text, allowed_symbols, enable_auto_lowercase=True):
    result = []
    for char in text:
...
def encrypt(key: list, data: str, allowed_symbols=None, enable_auto_lowercase=True):
    if allowed_symbols is None:
        allowed_symbols = A
    block_length = len(key)
    processed_text = preprocess_text(data, allowed_symbols, enable_auto_lowercase)
    temp_result: list[list] = []
    processed_text.extend([""] * (block_length - len(processed_text) % block_length))
    for i in range(len(processed_text) // block_length):
        temp_result.append([""] * block_length)
        output_block = temp_result[i]
        block = processed_text[i * block_length : i * block_length + block_length]
...
def decrypt(key: list, data: str, allowed_symbols=None, enable_auto_lowercase=True):
    if allowed_symbols is None:
        allowed_symbols = A
    block_length = len(key)
    processed_data = preprocess_text(data, allowed_symbols, enable_auto_lowercase)
    ...
    for i in range(block_length):
        ...
        if orig_column_pos < large_columns_count:
            ...
            for _ in range(len(processed_data) // block_length + 1):
                temp_result[orig_column_pos].append(next(processed_input))
        else:
            temp_result[orig_column_pos] = []
...
if __name__ == "__main__":
    ...
    if mode in encrypt_inputs:
        print("encrypted text: ", *encrypt(list(map(int, key.split())), text), sep="")
    elif mode in decrypt_inputs:
        print("decrypted text: ", *decrypt(list(map(int, key.split())), text), sep="")
    else:
        raise ValueError(
